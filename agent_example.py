...
def save_signals_to_json(signal_data: list, filename: str = "signals_data.json"):
    """신호 데이터를 JSON 파일로 저장"""
    try:
        # datetime 객체를 문자열로 변환 가능한 데이터로 변환
        serializable_data = []
        for signal in signal_data:
            if isinstance(signal, dict):
                ...
                serializable_data.append(serialized_signal)
            else:
                serializable_data.append(signal)
        
        # JSON 파일로 저장
        with open(filename, 'w', encoding='utf-8') as f:
            json.dump(serializable_data, f, ensure_ascii=False, indent=2)
        
        print(f"✅ 신호 데이터 저장 완료: {filename} ({len(signal_data)}개 신호)")
        return True
...
def generate_signal_data_with_indicators(price_data: pd.DataFrame, price_data_15m: pd.DataFrame, 
    ...
    end_idx = min(start_idx + max_periods, len(price_data))
    ...
    # 신호 데이터를 JSON 파일로 저장
    timestamp_str = price_data.index[start_idx].strftime('%Y%m%d_%H%M%S')
    filename = f"signals_data_{timestamp_str}.json"
    save_signals_to_json(signal_data, filename)
